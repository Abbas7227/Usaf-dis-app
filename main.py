...
def flight_plan_visualizer():
    ...
    c.CTkButton(header, text="Straight Line",
                ...
                width=100, height=30, fg_color=BG_LIGHT, hover_color=ACCENT).place(x=200, y=10)

    c.CTkButton(header, text="Freehand",
                command=lambda: draw_mode.update(mode="freehand"),
                width=100, height=30, fg_color=BG_LIGHT, hover_color=ACCENT).place(x=310, y=10)

    def change_color(choice):
        current_draw_color["color"] = draw_colors[choice]

    color_picker = c.CTkOptionMenu(header, values=list(draw_colors.keys()), command=change_color,
                                   width=100, fg_color=BG_LIGHT, button_color=ACCENT,
                                   text_color=TEXT)
    color_picker.set("Black")
    color_picker.place(x=420, y=10)

    # --- Map Area ---
    map_frame = c.CTkFrame(bg, fg_color=BG_MID, corner_radius=20, width=640, height=500)
    map_frame.place(x=30, y=70)

    map_label = c.CTkLabel(map_frame, image=map_state["ctk_image"], text="", fg_color="transparent", width=635, height=491)
